...
public final class PredictUtils {
    ...
    public static Vec3d predict(LivingEntity entity, Vec3d pos, float ticks) {
        if (Math.hypot(entity.getX() - entity.prevX, entity.getZ() - entity.prevZ) * 20.0D <= 5.0D
                && (entity.getY() - entity.prevY) * 20.0D <= 5.0D) {
            return pos;
        }
        ...
        float f = (float) (entity.getPitch() * 0.017453293005625408D);
        ...
        if (f < 0.0F && d > 0.0D) {
            i = e * -MathHelper.sin(f) * 0.04D;
            oldVelocity = oldVelocity.add(-lookVec.x * i / d, i * 3.2D, -lookVec.z * i / d);
        }
        if (d > 0.0D) {
            oldVelocity = oldVelocity.add(
                    (lookVec.x / d * e - oldVelocity.x) * 0.1D,
                    0.0D,
                    (lookVec.z / d * e - oldVelocity.z) * 0.1D);
        }

        Vec3d totalMotion = oldVelocity.multiply(0.99D, 0.98D, 0.99D);
        return pos.add(totalMotion.multiply(ticks)).add(0.0D, 0.35D, 0.0D);
    }
