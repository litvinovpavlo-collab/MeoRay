...
public class ESP extends Module {
    ...
    public final ModeSetting dimension = modeSection.add(new ModeSetting("Dimension", "2D", "2D", "3D"));
    public final ModeSetting style2D = modeSection.add(new ModeSetting("2D Style", "WexSide",
            "Corner", "Box", "WexSide", "Outline"));
    public final ModeSetting style3D = modeSection.add(new ModeSetting("3D Style", "Outline",
            "Outline", "Filled", "Both"));
    public final NumberSetting cornerLen = modeSection.add(new NumberSetting("Corner Length", 6.0, 2.0, 20.0, 1.0));
    public final NumberSetting lineWidth = modeSection.add(new NumberSetting("Line Width", 1.0, 0.5, 4.0, 0.5));
    public final NumberSetting fillAlpha = modeSection.add(new NumberSetting("Fill Alpha", 30.0, 0.0, 200.0, 5.0));
    ...
    public final BooleanSetting tMobs = targetsSection.add(new BooleanSetting("Mobs", true));
    public final BooleanSetting tAnimals = targetsSection.add(new BooleanSetting("Animals", false));
    public final BooleanSetting tInvisibles = targetsSection.add(new BooleanSetting("Invisibles", true));

    // === SECTION: VISUALS ===
    public final SectionSetting visualsSection = add(new SectionSetting("Visuals"));
    public final BooleanSetting vHealthBar = visualsSection.add(new BooleanSetting("Health Bar", true));
    public final BooleanSetting vNameTag = visualsSection.add(new BooleanSetting("Name Tag", true));
    public final BooleanSetting vArmor = visualsSection.add(new BooleanSetting("Armor", true));
    public final BooleanSetting vGlow = visualsSection.add(new BooleanSetting("Glow", true));
    public final BooleanSetting vFilled = visualsSection.add(new BooleanSetting("Filled", true));
    public final BooleanSetting vDistance = visualsSection.add(new BooleanSetting("Distance", true));
    public final BooleanSetting vTracers = visualsSection.add(new BooleanSetting("Tracers", false));

    // === SECTION: COLORS ===
    public final SectionSetting colorsSection = add(new SectionSetting("Colors"));
    public final ColorSetting boxColor = colorsSection.add(new ColorSetting("Box Color", new Color(180, 80, 255)));
    public final ColorSetting outlineColor = colorsSection.add(new ColorSetting("Outline Color", new Color(0, 0, 0, 180)));
    public final ColorSetting tracerColor = colorsSection.add(new ColorSetting("Tracer Color", new Color(180, 80, 255)));
    public final ColorSetting friendColor = colorsSection.add(new ColorSetting("Friend Color", new Color(0, 200, 80)));
    ...
    public void onRender2D(DrawContext ctx, float tickDelta) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (vDistance.getValue()) {
                drawDistance(ctx, living, x, y2, w);
            }

            if (vArmor.getValue() && living instanceof PlayerEntity player) {
    ...
    public void onWorldRender(MatrixStack matrices, float tickDelta) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (vTracers.getValue()) {
                int tc = tracerColor.getRGB() | 0xFF000000;
                Render3DUtils.drawTracer(matrices, box.getCenter(), tc, lineWidth.getValue().floatValue());
            }
        }
    }
    ...
    private void drawCleanHealthBar(DrawContext ctx, double x, double y, double h, float health, float maxHealth) {
        double barW = 2.5;
    ...
    private int getHealthColor(double percent) {
        if (percent > 0.6) return 0xFF55DD55;
    ...
    private void drawNameTag(DrawContext ctx, LivingEntity entity, double x, double y, double w, boolean friend) {
        ...
        String typeLetter;
        int typeCol;
        if (friend) { typeLetter = "F"; typeCol = friendColor.getRGB() | 0xFF000000; }
        else if (entity instanceof PlayerEntity) { typeLetter = "P"; typeCol = 0xFFFF4444; }
        else if (entity instanceof HostileEntity) { typeLetter = "M"; typeCol = 0xFFFFAA22; }
        else if (entity instanceof net.minecraft.entity.passive.AnimalEntity) { typeLetter = "A"; typeCol = 0xFF99DDFF; }
        else { typeLetter = null; typeCol = 0; }
        ...
        if (typeLetter != null) {
            ((BuiltText) Builder.text().font(font).text(typeLetter)
                .color(typeCol).size(textSize).thickness(0.06F).build())
                .render(matrix, contentX, contentY);
            contentX += letterW + gapAfterLetter;
        }
        int nameCol = friend ? (friendColor.getRGB() | 0xFF000000) : 0xFFFFFFFF;
        ((BuiltText) Builder.text().font(font).text(name)
            .color(nameCol).size(textSize).thickness(0.04F).build())
            .render(matrix, contentX, contentY);
        float hpPercent = entity.getHealth() / entity.getMaxHealth();
        int hpColor = getHealthColor(hpPercent);
        float hpX = contentX + nameW + spaceW;
        ((BuiltText) Builder.text().font(font).text(hpStr)
            .color(hpColor).size(textSize).thickness(0.04F).build())
            .render(matrix, hpX, contentY);
        float heartX = hpX + hpW + 1;
        ((BuiltText) Builder.text().font(font).text("HP")
            .color(hpColor).size(textSize).thickness(0.04F).build())
            .render(matrix, heartX, contentY);
    }
    ...
    private void drawDistance(DrawContext ctx, LivingEntity entity, double x, double y2, double w) {
        ...
        ((BuiltRectangle) Builder.rectangle()
            ...
            .smoothness(1.15F)
            .build()).render(matrix, tx, ty);
        ((BuiltText) Builder.text().font(font).text(text)
            .color(0xFFAACCFF).size(textSize).thickness(0.04F).build())
            .render(matrix, tx + padX, ty + padY);
    }
    ...
    private void drawArmor(DrawContext ctx, PlayerEntity player, double x, double y2, double w) {
        ...
        try {
            ...
            for (ItemStack s : player.getArmorItems()) if (s != null && !s.isEmpty()) items.add(s);
        ...
        float ay = (float) (y2 + (vDistance.getValue() ? 16 : 3));
    ...
    private int getEntityColor(LivingEntity entity) {
        ...
        if (entity instanceof HostileEntity) return 0xFFFF5555;
        return 0xFF55DD55;
    }
    ...
    private boolean isValid(Entity entity) {
        ...
        if (living.isInvisible() && !tInvisibles.getValue()) return false;
        if (entity instanceof PlayerEntity) return tPlayers.getValue();
        if (entity instanceof net.minecraft.entity.passive.AnimalEntity) return tAnimals.getValue();
        return tMobs.getValue();
    }
}
